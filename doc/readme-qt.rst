iWalletCoin-qt: Qt4 GUI for iWalletCoin
===============================
...
Alternatively, install Qt Creator and open the `iWalletCoin-qt.pro` file.

An executable named `iWalletCoin-qt` will be built.
...
A warning for people using the *static binary* version of iWalletCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of iWalletCoin is linked against libdb5.3.
...
installed causes iWalletCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.

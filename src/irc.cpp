...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("iWalletCoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #iWalletCoinTEST2\r");
            Send(hSocket, "WHO #iWalletCoinTEST2\r");
        } else {
            // randomly join #iWalletCoin00-#iWalletCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #iWalletCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #iWalletCoin%02d\r", channel_number).c_str());
        }
